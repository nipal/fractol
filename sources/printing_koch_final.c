...
/*   Updated: 2016/11/26 17:47:16 by fjanoty          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	 print_koch_fractale_anime(t_koch_const *kco, t_koch_changing kch, double anime)
{
	...
	while (kch.seg->next)
	{
		if (kch.iter >= kco->max_iter || (diff_2(kch.seg->pos, kch.seg->next->pos) < kco->min_print))
			...
			|| !(cpy->next = copy_node(kch.seg->next, kch.seg->next->lvl)))
			return ;
		else if ((to_insert = creat_insert_scale(cpy, kco->trans, 1, anime))
				&& (insert_portion(&(cpy), to_insert)))
			print_koch_fractale_anime(kco, init_kch(cpy, kch.iter + 1 , kch.dist + (du * i / len), du), anime);
		i += polygone_destroy(&(cpy)) > -1;
		kch.seg = kch.seg->next;
	}
}
