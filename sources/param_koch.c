...
/*   Updated: 2016/11/21 00:40:03 by fjanoty          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_polygone	*scale_segment(t_polygone *src, t_border *from, t_border *to, double *add)
{
	...
	while (src)
	{
		if (!(new_node = copy_node(src, 0)))
			return (NULL);
		new_node->pos->m[0] = (new_node->pos->m[0] - from->x0) * scale_x + add_s[0];
		new_node->pos->m[1] = (new_node->pos->m[1] - from->y0) * scale_y + add_s[1];
