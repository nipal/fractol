...
/*   Updated: 2016/11/19 04:10:47 by fjanoty          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
**	la on voudrai supprimer un maillon avec une clique droit
**
**		->	on recupere le noeud le plus proche.
**		->	on sauvgarde son adresse.
**		->	si il n'est ni dernier, ni premier:
**			- on le suprime
**
*/
...
int			remove_one_node(t_env *e)
{
	...
	to_remove = NULL;
	begin = NULL;
	if (mouse_in_border(&(e->border_b), e->param->mouse) && (begin = e->base_model))
		to_remove = get_closer_node(e->base_model, e->param->mouse, e->r_select);
	if (mouse_in_border(&(e->border_t), e->param->mouse) && (begin = e->trans_model))
		to_remove = get_closer_node(e->trans_model, e->param->mouse, e->r_select);
	if (!to_remove || !begin || to_remove == begin || to_remove->next == NULL)
		return (0);
	node = begin;
	while (node->next && node->next != to_remove)
		node = node->next;
	if (!node->next)
		return (0);
	tmp = node->next;
	...
	(begin == e->base_model) ? actu_base(e, e->base_model): actu_transform(e, e->trans_model);
	print_fractal(e);
