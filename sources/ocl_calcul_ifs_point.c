...
/*   Updated: 2017/04/22 03:36:44 by fjanoty          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ocl_mem_creat_calcul_ifs(t_ocl_ker *ifs_ker)
{
	...
	ret[2] = ocl_create_mem(ifs_ker, 2, CL_MEM_READ_ONLY, sizeof(int) * MAX_ITER);
...
int	ocl_writeto_ifs_calcul(t_ocl_ker *ifs_cl, t_ifs_ocl *data)
{
	...
	ret[2] = clEnqueueWriteBuffer(ifs_cl->command_queue, ifs_cl->data[2].gpu_buff, CL_TRUE, 0, sizeof(int) * MAX_ITER, tab_id, 0, NULL, NULL);
	/*
	ret2 =clGetCommandQueueInfo (cl_command_queue command_queue,
...
//	print_id_tab(id_tab, MAX_ITER);
	...
	while (i < nb_iter)
	{
		global_work_size[0] = id_tab[i + 1] - id_tab[i]; // Soit calcul une id de plus; Soit avoir une varible qui stoque le resulta
	 	ret2 = clEnqueueWriteBuffer(ifs_cl->command_queue, ifs_cl->data[4].gpu_buff, CL_TRUE, 0, ifs_cl->data[4].size, &(i), 0, NULL, NULL);
		if (ret2)
	...
	t_env *e = get_env(NULL);
	ret3 = clEnqueueNDRangeKernel(ifs_cl->command_queue, ifs_cl->kernel, 1, NULL, global_work_size, local_work_size, 0, NULL, NULL);
	printf("nb_iter:%d\n", nb_iter);
